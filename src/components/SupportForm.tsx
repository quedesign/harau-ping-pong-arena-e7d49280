  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSubmissionError(errorData.error || 'Failed to send email');
      }
    } catch (error: any) {
      setSubmissionError(error.message || 'Failed to send email');
    } finally {
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              ...
              <input
                ...
                className="w-full bg-zinc-800 text-gray-100 px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              />
            </div>
            <div className="mb-4">
